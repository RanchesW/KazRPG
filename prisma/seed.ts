...
async function main() {
  ...
  const game1 = await prisma.game.create({
    data: {
      ...
      tags: JSON.stringify(['Новичкам', 'Фэнтези', 'Онлайн', 'Кампания']),
      isOnline: true,
  ...
  const game2 = await prisma.game.create({
    data: {
      ...
      tags: JSON.stringify(['Хоррор', 'Расследование', 'Казахстан', 'Атмосфера']),
      isOnline: false,
      city: 'Алматы',
  ...
  const game3 = await prisma.game.create({
    data: {
      ...
      duration: 300,
      difficulty: 'ADVANCED',
      tags: JSON.stringify(['Казахская мифология', 'Эпик', 'Pathfinder']),
      isOnline: true,
